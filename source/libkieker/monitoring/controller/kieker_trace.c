...
pthread_rwlock_t kieker_trace_hash_lock;
...
kieker_trace_t* kieker_trace_get() {
	...
	/* get trace data */
	int ret = pthread_rwlock_rdlock(&kieker_trace_hash_lock);
	if (ret != 0)
		KIEKER_ERROR_PLAIN("Could not acquire rwlock!");
	kieker_trace_t *data = kieker_trace_find(thread_id);
	pthread_rwlock_unlock(&kieker_trace_hash_lock);
	...
	if (data == NULL) {
		...
		ret = pthread_rwlock_wrlock(&kieker_trace_hash_lock);
		if (ret != 0)
			KIEKER_ERROR_PLAIN("Could not acquire rwlock!");
...
void kieker_trace_remove(kieker_trace_t *data) {
	int ret = pthread_rwlock_wrlock(&kieker_trace_hash_lock);
	if (ret != 0)
		KIEKER_ERROR_PLAIN("Could not acquire rwlock");
...
int kieker_trace_init() {
	int ret = pthread_rwlock_init(&kieker_trace_hash_lock, NULL);
	if (ret != 0) {
		KIEKER_ERROR_PLAIN("Could not create rwlock!");
		return 0;
	}
...
void kieker_trace_finalize() {
	pthread_rwlock_destroy(&kieker_trace_hash_lock);
}
