...
void kieker_probe_before_operation_record(const char* class_signature, const char* operation_signature) {
	...
	if (entry->order_index == 0) { // new thread
		...
		offset += kieker_common_record_flow_trace_trace_metadata_serialize(kieker_controller_buffer,
	...
	offset += kieker_common_record_flow_trace_operation_before_operation_event_serialize(kieker_controller_buffer,
			KIEKER_FLOW_BEFORE_OPERATION, offset, entry);
...
void kieker_probe_after_operation_record(const char* class_signature, const char* operation_signature) {
	...
	int offset = kieker_common_record_flow_trace_operation_before_operation_event_serialize(kieker_controller_buffer,
			KIEKER_FLOW_AFTER_OPERATION, 0, entry);
