...
#include <stdlib.h>
#include <kieker.h>
#include "before_database_event.h"
 ...
 */
int kieker_common_record_database_before_database_event_serialize(char *buffer, const int id, const int offset, const kieker_common_record_database_before_database_event value) {
	int length = 0;
	...
	length += kieker_serialize_string(buffer, offset, value.parameters);
	length += kieker_serialize_string(buffer, offset, value.technology);
